        ...
        function bgParallax() {
            ...
            if (bgItems.length) document.addEventListener("windowScroll", (function(e) {
                bgItems.forEach((bgItem => {
                    let size = bgItem.dataset.bg ? Number(bgItem.dataset.bg) : 70;
                    ...
                    let bgItemScrollPercent = Math.abs((bgItem.getBoundingClientRect().top - window.innerHeight) / (bgItemHeight + window.innerHeight) * 100);
                    let bgItemPositionValue = bgItemHeight / 100 * size / 100 * bgItemScrollPercent;
        ...
        window.addEventListener("load", (function(e) {
            ...
            if (document.querySelector(".video-module")) {
                document.addEventListener("watcherCallback", (function(e) {
                    const entry = e.detail.entry;
                    const targetElement = entry.target;
                    if (targetElement.dataset.watch === "video") if (entry.isIntersecting) targetElement.querySelector("video").play(); else targetElement.querySelector("video").pause();
                }));
                document.querySelector(".video-module").addEventListener("click", (function(e) {
                    const divVideoMudule = document.querySelector(".video-module");
                    const tagVideo = divVideoMudule.querySelector("video");
                    if (!divVideoMudule.classList.contains("_init")) {
                        tagVideo.src = tagVideo.dataset.full;
                        divVideoMudule.classList.add("_active-video");
                        divVideoMudule.classList.add("_init");
                        tagVideo.play();
                        tagVideo.muted = false;
                    } else {
                        tagVideo.pause();
                        divVideoMudule.classList.remove("_active-video");
                    }
                }));
            }
        }));
